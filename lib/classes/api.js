        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (apiRequest.authTokenName) {
                ...
                if (!apiRequest.authenticated) {
                    middleware.sendResponse(http_error_codes_1.HttpErrors[401], 401, 'text/plain');
                }
            }
            const func = api.controllers.get(apiRequest.routeName, method);
            try {
                apiRequest.send = (bodyContent, status, contentType) => {
                    const responseDetail = get_response_detail_1.getResponseDetail(bodyContent, status, contentType);
                    middleware.sendResponse(responseDetail.body, responseDetail.statusCode, responseDetail.contentType);
                };
                yield func.call(undefined, apiRequest);
            }
            catch (ex) {
                Api.handleRouteError(ex, middleware.sendResponse.bind(middleware), api.options.custom404);
                return;
    ...
    static handleRouteError(ex, sendResponse, custom404) {
        switch (ex.name) {
            case 'RotiroErrorResponse':
                ...
                sendResponse(responseError.content ||
                    responseError.message ||
                    http_error_codes_1.HttpErrors[responseError.status] ||
                    'Api Error', responseError.status, 'text/plain');
                return;
            case 'RotiroError':
                if (ex.errorCode === 101) {
                    if (!custom404) {
                        sendResponse(http_error_codes_1.HttpErrors[404], 404, 'text/plain');
                    }
                    else {
                        throw ex;
                    }
                }
                sendResponse(http_error_codes_1.HttpErrors[500], 500, 'text/plain');
                return;
        }
    }
    build() {
